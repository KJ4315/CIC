...
function Find() {
    ...
    const [nameMessage, setNameMessage] = useState(''); // Name Valid Check Message - Find Id
    const [_nameMessage, _setNameMessage] = useState(''); // Name Valid Check Message - Find Pw
    ...
    const checkInutValue = (e) => {
        ...
        if(check === -1){
            ...
            }else if(inputId==='name_pw'){
                _setNameMessage("사용할 수 없는 문자입니다.");
                document.getElementById(inputId).focus();
            }else if(inputId==='id'){
    ...
    return  <Container>
                ...
                >
                    <SubContainer key={1} bg='' borderRight='1px solid #E0E0E0' onSubmit={(e)=>findIdSubmit(e)}>
                        <Title>아이디 찾기</Title>
                        <Input indent='15px' id='name_id' type='text' placeholder='이름을 입력해주세요.' required onBlur={(e)=>checkInutValue(e)} />
                    ...
                    <SubContainer key={2} bg='' onSubmit={(e)=>findPwSubmit(e)}>
                        ...
                        <Input indent='15px' id='name_pw' type='text' placeholder='이름을 입력해주세요.' required onBlur={(e)=>checkInutValue(e)} />
                        <ErrorMessage>{_nameMessage}</ErrorMessage>
                        <Input margin='0 0 30px 0' indent='15px' id='phone' type='tel' placeholder="'-' 빼고 입력해주세요." pattern="[0-9]{3}[0-9]{4}[0-9]{4}"/><br/>
